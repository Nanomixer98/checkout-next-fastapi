...
export class CheckoutPresenter {
   ...
   */
  async processPayment(customerInfoData: CustomerInfo, paymentInfoData: PaymentInfo): Promise<PaymentResponseDTO> {
    ...
    // Procesar pago
    const result = await this.processPaymentUseCase.execute(customer, payment, order)
